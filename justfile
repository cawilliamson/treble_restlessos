...
build-rom: build-container
    {{CONTAINER_RUN}} -w /repo/src gsi-builder \
        /bin/bash -e -c ' \
            ...
            find out/soong/.intermediates/prebuilts/sdk/ -type d \( -name aapt2 -o -name flat-res \) -exec rm -rfv {} + 2>/dev/null; true && \
...
copy-to-webdir: build-container
    {{CONTAINER_RUN}} -w /repo/tmp gsi-builder \
        /bin/bash -e -c ' \
            ...
            mkdir -p "/web/${RELEASE_NAME}/zsync" && \
            cp -fv /repo/out/*.img.xz "/web/${RELEASE_NAME}/" && \
            cp -fv /repo/out/zsync/*.img "/web/${RELEASE_NAME}/zsync/" && \
            cp -fv /repo/out/zsync/*.img.zsync "/web/${RELEASE_NAME}/zsync/" && \
